...
function getResolutions(extent) {
...
export function createTileGrid(extent, origin, tileSize = 256) {
    return new TileGrid({
...
export function createTileLayer(url, projection) {
...
export class TileSuperMapRest extends TileImage {
    ...
    tileUrlFunctionSM(tileCoord, pixelRatio, projection) {
        const z = tileCoord[0];
        const x = tileCoord[1];
        const y = tileCoord[2];
        ...
        let url = this.mapUrl + encodeURI(`&x=${x}&y=${y}&width=${tileSize[0]}&height=${tileSize[1]}&scale=${scale}`);
    ...
    setTileSetsInfo(tileSets) {
        ...
        this.dispatchEvent(new TileChangeEvent({
